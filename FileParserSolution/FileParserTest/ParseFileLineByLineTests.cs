...
{
    public class ParseFileLineByLineTests
    {
        [Fact]
        void BasicTest()
        {
            ...
            using (LineByLineParsedFile file = FileReader.ParseFileLineByLine(fileName))
            {
                Queue<string> firstLine = file.FileQueue.Dequeue();
                initialNumber = FileReader.Extract<int>(firstLine);
                category = FileReader.Extract<string>(firstLine);

                int line_counter = 0;
                while (file.FileQueue.Count > 0)
                {
                    List<int> listNumbers = new List<int>();

                    Queue<string> line = file.FileQueue.Dequeue();

                    int n_numbers = FileReader.Extract<int>(line);
